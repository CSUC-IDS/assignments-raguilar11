...
Make sure to adjust fig.height to keep figs on the same page of output as the code that creates them!
```
```{r, fig.height = 3.5}
sim1a <- tibble(
  x = rep(1:10, each = 3),
...
``` 
```{r, echo = FALSE, eval = FALSE}
For the above look at your error output and see what you could have changed in your code to remove the error messages in the final .pdf
```
...
```{r, echo = FALSE, eval = FALSE}
For these plots consider playing around with fig.height to make them more square.  You can see the lines quite well as it but the aspect ratio just feels wrong visually.
```
```{r}
...
```{r, echo = FALSE, eval = FALSE}
Good work, looks really clean. :thumbs_up:
```